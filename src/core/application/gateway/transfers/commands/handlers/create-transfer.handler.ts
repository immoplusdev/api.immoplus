...
import { DestinationMobile, Origin, TransfertPayloadDto } from "@/core/domain/gateways/transfers/transfert-payload.dto";
import { Hu2TransferResponseDto } from "@/core/domain/gateways/transfers/hu2-transfer-response.dto";
import { getProviderHub2, getTransferMethodFees } from "../../../utils/hub2.utils";

@CommandHandler(CreateTransferCommand)
export class CreateTransferHandler implements ICommandHandler<CreateTransferCommand> {
    constructor(
        @Inject(Deps.GatewayRepository) private readonly gatewayRespository: IGatewayRepository,
    ...
    async execute(command: CreateTransferCommand): Promise<Transfer> {
        const withdrawal = await this.walletRepository.findWalletWithdrawalRequestById(command.walletWithDrawalRequestId);
        ...
        /** Get user info */
        const userId = typeof withdrawal.owner === "string" ? withdrawal.owner : withdrawal.owner.id;
        ...
        const fees = await getTransferMethodFees(withdrawal.amount, withdrawal.operator);
        ...
        try {
            const hub2Response: Hu2TransferResponseDto = await this.gatewayRespository.createTransfer(transferPayload);
        ...
        this.walletRepository.updateWalletWithdrawalRequest(withdrawal.id, {status : WithdrawalStatus.APPROVED});
