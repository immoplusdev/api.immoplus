...
import { IAuthService, ITfaService, UserCannotLoginException } from "@/core/domain/auth";
...
export class LoginWithEmailOtpCommandHandler implements ICommandHandler<LoginWithEmailOtpCommand> {
  ...
  async execute(command: LoginWithEmailOtpCommand): Promise<LoginWithEmailOtpCommandResponse> {
