...
export class AnnulerDemandeVisiteByIdCommandHandler
  implements ICommandHandler<AnnulerDemandeVisiteByIdCommand>
{
  constructor(
  ...
  async execute(
    command: AnnulerDemandeVisiteByIdCommand,
  ): Promise<AnnulerDemandeVisiteByIdCommandResponse> {
    const demandeVisite = await this.demandeVisiteRepository.findOne(
      command.demandeVisite,
      { fields: ["id", "statusDemandeVisite"] },
    );
    ...
    try {
      ...
      return await this.queryBus.execute(
        new GetDemandeVisiteByIdQuery({ id: command.demandeVisite }),
      );
    }
    ...
    await this.demandeVisiteRepository.updateOne(
      command.demandeVisite,
      payload,
    );
