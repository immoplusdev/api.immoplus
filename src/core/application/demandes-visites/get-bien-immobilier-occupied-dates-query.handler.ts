...
export class GetBienImmobilierOccupiedDateQueryHandler
  implements
    IQueryHandler<
      ...
      GetBienImmobilierOccupiedDatesQueryResponse
    >
{
  constructor(
  ...
  async execute(
    query: GetBienImmobilierOccupiedDatesQuery,
  ): Promise<GetBienImmobilierOccupiedDatesQueryResponse> {
    ...
    const dates = reservations.data
      .map((demandeVisite) => demandeVisite.datesDemandeVisite)
      .flat();
