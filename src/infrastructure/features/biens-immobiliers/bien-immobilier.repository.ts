...
import { SearchBienImmobiliereGeoItemsParams, SearchGeolocalizedItemsParams, SearchItemsParams } from "@/core/domain/http";
...
export class BienImmobilierRepository implements IBienImmobilierRepository {
  ...
  async findByGeolocation(query: SearchBienImmobiliereGeoItemsParams): Promise<WrapperResponse<BienImmobilier[]>> {
        ...
        const qb = this.dataSource.getRepository(BienImmobilierEntity)
          .createQueryBuilder("bien")
          .leftJoinAndSelect("bien.miniature", "miniature", "miniature.deleted_on IS NULL")
          .leftJoinAndSelect("bien.video", "video", "video.deleted_on IS NULL")
          .leftJoinAndSelect("bien.ville", "ville", "ville.deleted_at IS NULL")
          .leftJoinAndSelect("bien.commune", "commune", "commune.deleted_at IS NULL")
          .leftJoinAndSelect("bien.proprietaire", "proprietaire", "proprietaire.deleted_at IS NULL")
        ...
        return new WrapperResponse(this.repository['mapResponse'](bienImmobiliers)).paginate({
          currentPage,
          pageSize,
          totalCount: total,
        });
  ...
  async findByGeolocationFilter(query: SearchGeolocalizedItemsParams): Promise<WrapperResponse<BienImmobilier[]>>{
     const DEFAULT_PAGE = 1;
        const DEFAULT_PAGE_SIZE = 10;
