...
export class NotificationService implements INotificationService {
  ...
  private async getOneSignalCredentials(userRole: UserRole) {
    ...
    return {
      app_id: this.configsManagerService.getEnvVariable(
        `ONE_SIGNAL_${keyPrefix}_APP_ID`,
      ),
      api_key: this.configsManagerService.getEnvVariable(
        `ONE_SIGNAL_${keyPrefix}_API_KEY`,
      ),
    };
  }
}
